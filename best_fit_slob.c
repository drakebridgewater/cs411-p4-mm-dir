...
static void *slob_page_alloc(struct slob_page *sp, size_t size, int align)
{
    ...
    int bestdelta, bestunits, delta = 0, units = SLOB_UNITS(size);
    slobidx_t avail, bestavail = -1;
    ...
    for (prev = NULL, cur = sp->free; ; prev = cur, cur = slob_next(cur)) {
        ...
        if (avail >= units + delta) { /* room enough? */
            if (bestavail != -1 || avail < bestavail){ /* IS Better fit */
                best = cur;
                bestprev = prev;
                bestaligned = aligned;
                bestdelta = delta;
                ...
                printk("[SLOB.C] FIRST FIT AREA UNUSED: ",ffavail - ffsize);
            }
        }
    ...
    /* SET VARIABLES TO PREVIOUS NAME */
    cur = best;
    prev = bestprev;
    aligned = bestaligned;
    delta = bestdelta;
    ...
    printk("[SLOB.C] BEST FIT AREA UNUSED: ",avail - units -delta);
